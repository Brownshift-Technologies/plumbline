...
Ten of Plumbline's eleven agents call `gemini-3.5-flash`. The Runner — the one that actually
executes a Playwright spec and reports pass, fail, timeout, or selector-not-found — calls
none. That's deliberate, not an oversight: if execution itself involved a model call, a
