...
from core.events import InvalidPushError, parse_pubsub_push
from core.guards import redact_pii
...
# first is in `core.guards`, where `_EMAIL`'s repeats are bounded so the
# pattern is linear rather than quadratic; that is the real fix and it also
# covers `core.store`, which redacts every audit write. This cap is here
...
def _describe(exc: BaseException) -> str:
    ...
    payload is redacted on its way into Firestore by `core.store`. An
    ...
    pattern itself was bounded in `core.guards` (the load-bearing fix,
    since `core.store` redacts on the audit path too). Capping before
    ...
    match entirely (see core.guards for why that combination, and not
...
def create_app(on_event: EventHandler, service_name: str) -> FastAPI:
    ...
    - `event.parse_failed` -- the parser raised something that is *not*
      `InvalidPushError`. `core.events` is explicit that its list of
      ...
      guarantee is not. ERROR rather than WARNING -- it means a core
    ...
    said so" is not a mechanism, and this core is vendored into three
