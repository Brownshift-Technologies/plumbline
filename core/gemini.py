...
# core.fakes.FakeModel -- it takes `images: list[bytes]`, nothing else --
...
class GeminiModel:
    ...
    Interface matches ``core.fakes.FakeModel`` exactly -- ``FakeModel``
    ...
    def __init__(self, config: Config, client=None, timeout_ms: int = DEFAULT_TIMEOUT_MS):
        ...
        if client is None:
            # Deferred, mirroring core.store.Store's pattern for its
            ...
            # only from Vertex's `global` endpoint (see core/config.py).
